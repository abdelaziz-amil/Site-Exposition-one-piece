...
<?php
    if ($_POST['vis_id'] && $_POST['vis_mdp'] && $_POST['vis_nom'] && $_POST['vis_prenom'] && $_POST['vis_mail'] && $_POST['commentaire']){
        ...
        if (!$mysqli->set_charset("utf8")) {
            ...
            AND VIS_NUMERO = ".$id."
            AND VIS_MOTDEPASSE = MD5('".$mdp."');";
            // Affichage de la requête constituée pour vérification
            //Exécution de la requête d'ajout d'un compte dans la table des comptes
            ...
            {
                $requetetest = "SELECT VIS_NUMERO FROM T_VISITEUR_VIS WHERE VIS_NUMERO = ".$id."";
                $result1 = $mysqli ->query($requetetest);
                $ligne = $result1->num_rows;
                if ($ligne ==1){
                  $requete1="UPDATE T_VISITEUR_VIS SET VIS_NOM = '".$nom."', VIS_PRENOM = '".$prenom."', VIS_EMAIL = '".$mail."' WHERE T_VISITEUR_VIS.VIS_NUMERO = ".$id.";";
                  $result1 = $mysqli->query($requete1);
                  $requete2 = "INSERT INTO T_COMMENTAIRE_CMT VALUES(NULL,CURRENT_TIME(),'".$com."','".$id."','C');";
