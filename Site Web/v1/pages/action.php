...
<?php
   if ($_POST['id'] && $_POST['mdp'] && $_POST['v'] && $_POST['nom'] && $_POST['prenom'] && $_POST['mail']){
        ...
        if (!$mysqli->set_charset("utf8")) {
            ...
            //requête
            $sql1="INSERT INTO T_COMPTE_COM VALUES('" .$id. "','" .md5($mdp). "');";
            $sql2="INSERT INTO T_PROFIL_PFL VALUES('".$nom. "','".$prenom."','".$mail."','D','O',CURRENT_DATE,'".$id."');";
            $sql3="delete from T_PROFIL_PFL WHERE COM_PSEUDO = '".$id."';";
            $sql4="delete from T_COMPTE_COM WHERE COM_PSEUDO = '".$id."';";
            //Exécution de la requête d'ajout d'un compte dans la table des comptes
            $result1 = $mysqli->query($sql1);
            $result2 = $mysqli->query($sql2);
            if (strcmp($mdp,$vmdp)==0) //Erreur lors de l’exécution de la requête
            {
                echo "<br />";
                echo "<h6>Inscription réussie !</h6>" . "\n";
            }
            else if ($result1 == false || $result2==false)//Requête réussie
            {
