<?php
    ...
    if ($VouC == 'V'){
        $requete1 = "SELECT * FROM T_VISITEUR_VIS where VIS_NUMERO= '" . $visnum . "';";//recuperation du visiteur qui a pour numero $visnum
        $resultat1 = $mysqli->query($requete1);
        if ($resultat1->num_rows == 1) {
            $requete2 = "UPDATE `T_COMMENTAIRE_CMT` SET CMT_ETAT = 'V' WHERE T_COMMENTAIRE_CMT.VIS_NUMERO = ".$visnum.";";//changement de l'etat du commentaire par V (visible)
        ...
        $resultat1 = $mysqli->query($requete1);
        if ($resultat1->num_rows == 1) {
            $requete2 = "UPDATE `T_COMMENTAIRE_CMT` SET CMT_ETAT = 'C' WHERE T_COMMENTAIRE_CMT.VIS_NUMERO = ".$visnum.";";//changement de l'etat du commentaire par C (caché)
            $resultat2 = $mysqli->query($requete2);
            header("Location:admin_tickets.php");
        } else {
            header("Location:admin_tickets.php");
