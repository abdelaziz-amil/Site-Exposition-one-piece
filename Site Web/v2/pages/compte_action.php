<?php
    ...
    if($_POST['pseudo'] && $_POST['AouD']){
        ...
        if($etat == 'A' || $etat == 'D'){//verification de l'input de l'utilisateur
            if ($resultat1->num_rows == 1) {
                $requete2 = "UPDATE `T_PROFIL_PFL` SET `PFL_VALIDITEPROFIL` = '" . $etat . "' where `T_PROFIL_PFL`.`COM_PSEUDO` = '" . $id . "';";//changement d'etat du profil par A(activé) ou D(désactiver)
