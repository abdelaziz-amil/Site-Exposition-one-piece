...
<?php $mysqli = new mysqli('localhost','zamilab00','rj5qlcwn','zfl2-zamilab00');
     ...
     $id=$_GET['id'] ;
          $requete_oeuvre="select * from T_OEUVRE_OEU join TJ_PRESENTE_PRE using (OEU_CODE) join T_EXPOSANT_EXP using (EXP_ID) WHERE OEU_CODE = ".$id.";";//recupération de toutes les informations sur une oeuvre qui a pour code $id pour l'utilisé dans le title
...
<div class="wrapper row2">
  <main class="hoc container clear"> 
    ...
    <div class="content"> 
     ...
     $id=$_GET['id'] ;
          $requete_oeuvre="select * from T_OEUVRE_OEU join TJ_PRESENTE_PRE using (OEU_CODE) join T_EXPOSANT_EXP using (EXP_ID) WHERE OEU_CODE = ".$id.";";//recupération de toutes les informations sur une oeuvre qui a pour code $id pour les afficher sur la page
          $resultat1 = $mysqli->query($requete_oeuvre);
          if($resultat1 == false){
            echo"erreur la requete a échoué";
            echo"Errno".$mysqli->errno."\n";
          ...
          while($row = $resultat1->fetch_assoc()){
            echo '<h1><a href = exposant.php?id='.$row['EXP_ID'].'>Exposant : '.$row['EXP_NOM'].'</a></h1>';
          }
